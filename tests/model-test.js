...
assert.deepEqual(Model.parseFormatPreference(" epub, AZW3,epub, pdf "), ["EPUB", "AZW3", "PDF"])
assert.deepEqual(Model.parseFormatPreference(""), ["EPUB", "AZW3", "PDF", "MOBI"])
...
assert.equal(Model.combineSearch("dune", "formats:=EPUB"), "(dune) and (formats:=EPUB)")
assert.equal(Model.combineSearch("", "cover:false"), "cover:false")
assert.equal(Model.combineSearch("title:Dune", ""), "title:Dune")
